  ...
  http.createServer(function(req, res) {
    ...
    request.get(USERAGENTAPISTART + userAgentString + USERAGENTAPIEND, function(err, xhr, body) {
      ...
      request.get('http://caniuse.com/flexbox', function(err, xhr, body) {
        $ = cheerio.load(body);
        var supported = $("h4.browser--" + browser + " + ol").find('li:contains("' + version + '")').attr('title').toLowerCase().indexOf("not supported") === -1;
        var whetherOrNot = supported ? "": "not ";
        res.write("Your browser does " + whetherOrNot + "support flexbox.")
        res.end();
      })
    });
  }).listen(port);
